    ...
    <header>
      ...
      <div class="params">
        ...
        <el-tag v-for="(item, index) of svcParams" :key="index" size="mini">
          <p @dragstart="drag(item.paramName)" draggable="true" style="margin: 0;">{{item.paramName}}</p>
        </el-tag>
      </div>
    </header>
    <main @drop="onDrop">
...
export default {
  ...
  methods: {
    ...
    selectLeaf (data) {
      ...
      if (leaf) { this._svcParam(bizSvcMethodId) }
    }
  }
}
</script>
