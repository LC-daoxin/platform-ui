<template>
  <el-dialog
    title="用户组授权"
    :visible.sync="dialogGroupPermissionVisible"
  ...
  >
    <el-row type="flex" justify="center" align="middle">
      ...
      <el-col :span="11">
        <div class="content-box">
          <h3 class="tableTitle">用户组授权列表</h3>
          <!-- 用户组授权列表 start -->
          <el-table
            size="mini"
            :data="oldTableData"
...
export default {
  ...
  data () {
    return {
      ...
      oldIds: [], // 更新前的关联角色id
      newIds: [], // 更新后的关联角色id
      currentGroupID: '', // 当前选择用户组ID
      permissionType: 'Organization' // 权限类型
    }
  },
  methods: {
    // Tree回调
    ...
    // 关闭对话框
    handleDialog () {
      this.$emit('showDialog', 'GroupPermission')
    },
    // 打开对话框时，回调
    open () {
      if (this.currentRow) {
        this.getRoleGroupInfo(this.currentRow.ROLEID)
      }
    },
    // 获取当前角色与用户组现关联列表
    getRoleGroupInfo (roleId) {
      this.loading2 = true
      this.axios_M2.get(`/permission/oldOrganizationCheck/${roleId}`)
        .then((res) => {
          let data = res.data
    ...
    updateSystem () {
      ...
      this.axios_M2.put(`/permission/roleOrganizationCheck/${this.currentRow.SYSID}/${this.currentRow.ROLEID}/${this.permissionType}`, data)
        .then((res) => {
          console.log(data)
